...
os.environ['XERO_REDIRECT_URI'] = 'https://product-pulse-12.preview.emergentagent.com/xero-oauth-callback'
os.environ['FRONTEND_URL'] = 'https://product-pulse-12.preview.emergentagent.com'
    ...
    try:
        ...
        for order in completed_orders:
            ...
            for log in production_logs:
                stage = log.get("to_stage") or log.get("from_stage")
            ...
            # Also check for additional cores produced
            additional_cores = await db.additional_cores.find({
                "order_id": order_id
            }).to_list(length=None)
            ...
            for core in additional_cores:
                qty = core.get("quantity", 0)
                job_stock_quantity += qty
                stock_summary.append({
                    "product_description": f"Additional Core - {core.get('width', 'Unknown')}mm",
                    "quantity": qty,
