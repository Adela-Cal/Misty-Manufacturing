          ...
          {canEdit() && (
            <>
              ...
              {timesheet?.status === 'draft' && (
                ...
                >
                  {isAdminOrManager && selectedEmployeeId ? 'Submit for Approval' : 'Submit Timesheet'}
                </button>
              )}

              {!isManager && (timesheet?.status === 'submitted' || timesheet?.status === 'approved') && (
                <button
                  onClick={handleEditAndResubmit}
                  disabled={submitting}
                  className="bg-blue-500 hover:bg-blue-600 text-white font-medium px-4 py-2 rounded-lg transition-colors duration-200 flex items-center"
                  data-testid="edit-resubmit-timesheet"
                >
