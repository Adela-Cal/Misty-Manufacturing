#!/usr/bin/env python3
"""
Fix duplicate order numbers in the database
This script finds orders with duplicate order numbers and renumbers them
"""
...
async def fix_duplicate_order_numbers():
    ...
    for order_number, duplicate_orders in duplicates.items():
        # Keep the first order with its original number
        # Renumber the rest
        print(f"\nProcessing {order_number}:")
        
        for i, order in enumerate(duplicate_orders):
            if i == 0:
                print(f"  ✓ Keeping original: {order['id']}")
                continue
            ...
            ).sort("order_number", -1).to_list(length=None)
            ...
            for attempt in range(100):
                ...
                existing = await db.orders.find_one({"order_number": new_order_number})
                if not existing:
                    break
...
async def main():
    try:
        await fix_duplicate_order_numbers()
    finally:
        client.close()
